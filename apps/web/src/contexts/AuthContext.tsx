...
import { convertToCamelCaseRecursive } from '@fide-calculator/shared';
  ...
  const fetchProfiles = useCallback(async (userId: string) => {
    try {
      ...
      if (error) {
        ...
        const convertedProfiles = data.map(profile => convertToCamelCaseRecursive(profile) as UserProfile);
