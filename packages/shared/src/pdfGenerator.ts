...
import type { BackupData, Result } from './types';
...
/**
 * Build a jsPDF document for the given month backup (title, pie chart, rating change box, games table, footer).
 * Does not call .save() — caller decides how to use the doc (e.g. doc.save() on web, doc.output('dataurlstring') on mobile).
 */
export function buildPdfDocument(backup: BackupData): jsPDF {
  ...
  const chartX = leftBoxX + 30 + chartRadius;
  ...
  let boxColor = '#6b7280';
  if (backup.totalChange > 0) boxColor = '#22c55e';
  else if (backup.totalChange < 0) boxColor = '#ef4444';
  doc.setDrawColor('#e5e7eb');
  ...
  backup.data.forEach((game, rowIndex) => {
    ...
    tableColumns.forEach((col) => {
      ...
      const text = String(
        cellData === 'win' || cellData === 'loss' || cellData === 'draw'
          ? (cellData as string).toUpperCase()
          : cellData
      );
  ...
  try {
    ...
    doc.text(footerUrl, footerX + textWidth + spaceWidth, footerY);
  }
